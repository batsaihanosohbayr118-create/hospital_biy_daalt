...
export default function MedicalRecords({ selectedPatientId = null, selectedAppointmentId = null, selectedDoctorId = null, onGoPrescriptions }) {
  ...
  return (
    <div className="fade-up">
      ...
      {loading ? (
        <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:14, padding:'1rem', marginBottom:'1.5rem', color:'var(--muted)' }}>
          Ачаалж байна...
        </div>
      ) : list.length > 0 ? (
        <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:14, overflow:'hidden', marginBottom:'1.5rem' }}>
          <div style={{ display:'grid', gridTemplateColumns:'90px 1fr 1fr 1fr 130px', gap:'.75rem', padding:'.85rem 1rem', borderBottom:'1px solid var(--border)', color:'var(--muted)', fontWeight:700, fontSize:'.82rem' }}>
            <span>ID</span>
            <span>Онош</span>
            <span>Эмчилгээ</span>
            <span>Шинжилгээ</span>
          ...
          {list.map(record => (
            <div key={record.id} style={{ display:'grid', gridTemplateColumns:'90px 1fr 1fr 1fr 130px', gap:'.75rem', padding:'.9rem 1rem', borderBottom:'1px solid var(--border)', color:'var(--text)', fontSize:'.88rem' }}>
              <span>#{record.id}</span>
              <span>{record.diagnosis || '-'}</span>
              <span>{record.treatment || '-'}</span>
              <span>{record.test_results || '-'}</span>
              <span>{record.record_date ? String(record.record_date).slice(0, 10) : '-'}</span>
            </div>
          ))}
        </div>
      ) : patientId ? (
        <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:14, padding:'1rem', marginBottom:'1.5rem', color:'var(--muted)', textAlign:'center' }}>
          Эмнэлгийн бичлэг олдсонгүй.
        </div>
      ) : null}
